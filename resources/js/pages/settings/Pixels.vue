    ...
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <SettingsLayout>
            <div class="space-y-6">
                ...
                <div v-if="flash.ok" class="flex items-start gap-2 rounded-xl border border-success/30 bg-success/5 p-4 text-sm text-success">
                    <Check class="mt-0.5 size-4 shrink-0" />
                    {{ flash.ok }}
                </div>
                <div v-if="flash.error" class="flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
                    <X class="mt-0.5 size-4 shrink-0" />
                    {{ flash.error }}
                </div>

                <!-- The two mistakes that silently double a merchant's numbers.
                     Stated before the boxes, because after them is too late. -->
                <div class="space-y-3">
                ...
                <form class="space-y-6" @submit.prevent="save">
                    <div v-for="network in networks" :key="network.key">
                        <label class="mb-2 block font-mono text-lg font-semibold" :for="network.key">
                            {{ network.label }}
                        </label>
                        ...
                        <textarea
                            ...
                            class="field min-h-32 font-mono text-sm"
                            dir="ltr"
                            :placeholder="network.hint"
                        />

                        <p class="field-hint">
                            تقدر تضيف أكتر من بيكسل — كل معرّف (ID) في سطر.
                        </p>
                ...
                <!-- ── Conversions API ─────────────────────────────────────
                     Kept apart from the boxes above: it needs a token per Meta
                     pixel, and folding it in is what makes this page confusing.
                -->
                ...
                <!-- ── Match quality ───────────────────────────────────────
                     Meta grades this in Events Manager days later and never
                     says how to fix it. We compute the same thing from our own
                     orders today — and we know exactly which switch raises it.
                -->
                <section v-if="emq" class="surface p-5">
                    ...
                    <div class="mt-5 space-y-3">
                        <div v-for="signal in emq.signals" :key="signal.label">
                            ...
                            <!-- Only where it is actually low: advice next to a
                                 signal already at 100% is noise. -->
                ...
                <!-- What we already fire, so nobody adds it a second time by
                     hand — the mistake the red box above warns about. -->
                <section class="surface p-5">
                    <p class="font-medium">الأحداث اللي بنبعتها أوتوماتيك</p>
                    <ul class="mt-3 space-y-2 text-sm text-muted-foreground">
                        <li v-for="event in [
                            ...
                            <span>
                                <span class="font-mono text-xs text-foreground">{{ event[0] }}</span>
                                — {{ event[1] }}
                            </span>
                        </li>
                    </ul>
