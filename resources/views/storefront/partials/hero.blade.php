{{-- Hero slider.
     ...
     of a page bought with ad money, and a slider is not worth 40KB of it.

     Renders every slide server-side rather than fetching: slide two must be in
     the HTML so it is there the instant the customer swipes. --}}
@if ($slides->isNotEmpty())
<section class="relative overflow-hidden bg-muted" data-hero>
                ...
                <div class="mx-auto grid max-w-5xl items-center gap-6 px-5 py-10 md:grid-cols-2 md:gap-10 md:py-16">
                    ...
                    <div class="order-1 md:order-2">
                        <img src="{{ $slide->primaryImage()->url() }}"
                             ...
                             {{-- The largest thing above the fold on the first
                                  slide: it decides the LCP, and therefore the
                                  cost of every visit. --}}
        ...
        <div class="absolute inset-x-0 bottom-4 flex justify-center gap-2">
            @foreach ($slides as $slide)
                <button type="button" data-hero-dot="{{ $loop->index }}"
                        ...
                        class="h-1.5 rounded-full bg-foreground/25 transition-all data-[active]:w-6 data-[active]:bg-primary w-1.5"></button>
            @endforeach
        </div>
    @endif
</section>
...
(function () {
    ...
    // Nothing should be animating on a tab nobody is looking at.
